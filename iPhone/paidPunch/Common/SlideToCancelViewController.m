...
- (void)loadView {
	...
	UIImage *trackImage = [UIImage imageNamed:@"SlideBg.png"];
	...
	[slider setMinimumTrackImage:[UIImage imageNamed:@"Nothing.png"] forState:UIControlStateNormal];
	[slider setMaximumTrackImage:[UIImage imageNamed:@"Nothing.png"] forState:UIControlStateNormal];
	UIImage *thumbImage = [UIImage imageNamed:@"slidebtn.png"];
	...
	NSString *labelText = NSLocalizedString(@"Slide to use Punch", @"SlideToCancel label");
...
- (void) sliderUp: (UISlider *) sender
{
	...
	if (touchIsDown) {
		touchIsDown = NO;
        BOOL instructionsIsVisible = NO;
        id<UIApplicationDelegate> appDelegate = [[UIApplication sharedApplication] delegate];
        InstructionsView *tempView = (InstructionsView *)[appDelegate.window viewWithTag:205];
        if([tempView respondsToSelector:@selector(okBtnTouchUpInsideHandler:)])
            instructionsIsVisible = YES;
		if(instructionsIsVisible == NO){
...
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    NSLog(@"current=%f %f",currentLocation.x,currentLocation.y);
    NSLog(@"previous=%f %f",previousLocation.x,previousLocation.y);
    if(currentLocation.x > previousLocation.x)
    {
        ...
        if(currentLocation.x >300)
        {
            [slider setValue:slider.value+1.0];
        }
        if(currentLocation.x - previousLocation.x >10)
        {
            [slider setValue:slider.value+0.03];
        }
        else
        {
            [slider setValue:slider.value+0.02];
        }
