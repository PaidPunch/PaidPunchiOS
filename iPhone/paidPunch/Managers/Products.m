    ...
    [httpClient getPath:@"paid_punch/Products"
             parameters:nil
                success:^(AFHTTPRequestOperation *operation, id responseObject){
                    ...
                    [delegate didCompleteHttpCallback:kKeyProductsRetrieve success:TRUE message:[responseObject valueForKeyPath:kKeyStatusMessage]];
                }
                failure:^(AFHTTPRequestOperation* operation, NSError* error){
                    NSLog(@"Downloading new Products from server has failed.");
                    [delegate didCompleteHttpCallback:kKeyProductsRetrieve success:FALSE message:[Utilities getStatusMessageFromResponse:operation]];
                }
     ];
}
...
- (void) purchaseProduct:(NSObject<HttpCallbackDelegate>*) delegate index:(NSUInteger)index
{
    ...
    [httpClient putPath:@"paid_punch/Products"
             parameters:parameters
                success:^(AFHTTPRequestOperation *operation, id responseObject){
                    NSLog(@"Retrieved: %@", responseObject);
                    [delegate didCompleteHttpCallback:kKeyProductsPurchase success:TRUE message:[responseObject valueForKeyPath:kKeyStatusMessage]];
                }
                failure:^(AFHTTPRequestOperation* operation, NSError* error){
                    NSLog(@"Downloading new Products from server has failed.");
                    [delegate didCompleteHttpCallback:kKeyProductsPurchase success:FALSE message:[Utilities getStatusMessageFromResponse:operation]];
                }
     ];
}
