...
- (IBAction)showLocationSelector:(id)sender {
    ...
    if([[[UIDevice currentDevice] systemVersion] floatValue] >= 5.0){
        locationAlertView = [[UIAlertView alloc] initWithTitle:@"Enter your zip code."
                                                       message:nil
                                                      delegate:self
        ...
        locationAlertView = [[UIAlertView alloc] initWithTitle:@"Enter your zip code."
                                                       message:@"This shoudl be covered"
                                                      delegate:self
                                             cancelButtonTitle:@"Current location"
                                             otherButtonTitles:@"Okay", nil];
        alertViewTextField = [[UITextField alloc] initWithFrame:CGRectMake(12.0, 45.0, 260.0, 25.0)];
        [alertViewTextField setBackgroundColor:[UIColor whiteColor]];
        [alertViewTextField setReturnKeyType:UIReturnKeyDefault];
        alertViewTextField.delegate = self;
        [alertViewTextField becomeFirstResponder];
        [locationAlertView addSubview:alertViewTextField];
        
//        CGAffineTransform myTransform = CGAffineTransformMakeTranslation(0.0, 130.0);
//        [locationAlertView setTransform:myTransform];
    }
...
- (BOOL)alertViewShouldEnableFirstOtherButton:(UIAlertView *)alertView {
    if([[[UIDevice currentDevice] systemVersion] floatValue] >= 5.0){
        ...
        NSString *inputText = alertViewTextField.text;
        if([inputText length] >= 5){
            return YES;
        }
        else {
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 0) {
        ...
        if([[[UIDevice currentDevice] systemVersion] floatValue] >= 5.0){
            ...
            [userZipCode setString:alertViewTextField.text];
            NSUserDefaults *ud=[NSUserDefaults standardUserDefaults];
            [ud setObject:userZipCode forKey:@"userZipCode"];
            [ud synchronize];
            [self refreshBusinessList];
...
#pragma mark -
#pragma mark MKReverseGeocoderDelegate Methods Implementation

- (void)reverseGeocoder:(MKReverseGeocoder *)geocoder didFindPlacemark:(MKPlacemark *)placemark {
    [geoCoder cancel];
    
    NSLog(@"**Geocode successful; zip code: %@", [[placemark addressDictionary] objectForKey:@"PostalCode"]);
    [userZipCode setString:[[placemark addressDictionary] objectForKey:@"PostalCode"]];
    NSUserDefaults *ud=[NSUserDefaults standardUserDefaults];
    [ud setObject:userZipCode forKey:@"userZipCode"];
    [ud synchronize];
    [self refreshBusinessList];
